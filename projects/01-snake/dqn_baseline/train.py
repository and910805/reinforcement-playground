...
# 它包含 Colab 特有的 GDrive 掛載和 IPython 顯示功能
# -----------------------------------------------------------------
...
def train():
    ...
    CHECKPOINT_PATH = DRIVE_PATH + 'snake_checkpoint.pth' # 舊的檔名
    ...
    try:
        ...
        print(f"--- 成功從 Google Drive 加載 [基準版 DQN] 進度！---")
        print(f"--- 目前已訓練 {agent.n_games} 局，最高紀錄 {record} 分 ---")
        
    except FileNotFoundError:
        print(f"--- 找不到 [基準版 DQN] 進度檔，從頭開始訓練 ---")
    except Exception as e:
        print(f"--- 加載進度失敗: {e}，從頭開始訓練 ---")
    
    
    ROUNDS_PER_SESSION = 300  # 每次執行多練 300 局
    target_games = agent.n_games + ROUNDS_PER_SESSION
    
    print(f"--- 開始訓練 [基準版 DQN]！目標是從 {agent.n_games} 局練到 {target_games} 局 ---")
    time.sleep(3)
    ...
    while agent.n_games <= target_games:
        ...
        if done:
            game.reset()
            agent.n_games += 1
            agent.train_long_memory() # 睡前回想 (Long Memory)
            ...
            if agent.n_games % 20 == 0 or new_record:
                if new_record:
                    print(f"🎉 新紀錄！第 {agent.n_games} 局: {score} 分。儲存模型...")
                else:
                    print(f"--- 正在儲存進度到 Google Drive (局數 {agent.n_games}) ---")
