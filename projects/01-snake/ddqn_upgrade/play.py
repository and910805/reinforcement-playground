# [ 這是 ddqn_upgrade/play.py 的內容 ]

# -----------------------------------------------------------------
# 這是「升級版 DDQN」的 GIF 錄製程式
# -----------------------------------------------------------------
...
def record_ai_game_to_gif():
    ...
    # --- 1. 從 Google Drive 讀取「新的」DDQN 進度檔 ---
    DRIVE_PATH = '/content/drive/MyDrive/Colab_AI_Snake/'
    CHECKPOINT_PATH = DRIVE_PATH + 'snake_checkpoint_ddqn.pth' # 讀取 DDQN 檔名
    
    GIF_FILENAME = 'ai_snake_game_ddqn_upgrade.gif'
    GIF_SAVE_PATH = DRIVE_PATH + GIF_FILENAME
    
    print(f"--- 開始錄製「升級版 DDQN」AI 表演 (加載 {CHECKPOINT_PATH}) ---")
